...
void Screen::Init() {
    ...
    // CAMERA-SPACE POSITION
    {
        ...
        glGenTextures(1, &scene_camera_pos_texture);
        glBindTexture(GL_TEXTURE_2D, scene_camera_pos_texture);
        ...
        glFramebufferTexture(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT1, scene_camera_pos_texture, 0);
    }

    // WORLD-SPACE POSITION
    {
        // initializes the screen depth texture
        glGenTextures(1, &scene_world_pos_texture);
        ...
        // attaches the true depth texture to the framebuffer
        glFramebufferTexture(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT2, scene_world_pos_texture, 0);
    }
...
void Screen::ResizeCallback(GLsizei _width, GLsizei _height) {
    ...
    // resize the scene color texture
    glBindTexture(GL_TEXTURE_2D, scene_color_texture);
    glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, screen_width, screen_height, 0, GL_RGBA, GL_UNSIGNED_BYTE, nullptr);

    // resize the scene camera-space pos texture
    glBindTexture(GL_TEXTURE_2D, scene_camera_pos_texture);
    glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA16F, screen_width, screen_height, 0, GL_RGBA, GL_FLOAT, nullptr);

    // resize the scene world-space pos texture
    glBindTexture(GL_TEXTURE_2D, scene_world_pos_texture);
    glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA16F, screen_width, screen_height, 0, GL_RGBA, GL_FLOAT, nullptr);

    // resize the scene depth & stencil render buffer
    glBindRenderbuffer(GL_RENDERBUFFER, scene_depth_texture);
    glRenderbufferStorage(GL_RENDERBUFFER, GL_DEPTH24_STENCIL8, screen_width, screen_height);
}
